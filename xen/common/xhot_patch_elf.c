...
const struct xhot_patch_elf_sec *
xhot_patch_elf_sec_by_name(const struct xhot_patch_elf *elf, const char *name)
{
    unsigned int i;
...
static int elf_verify_strtab(const struct xhot_patch_elf_sec *sec)
{
    const Elf_Shdr *s;
    const char *contents;
...
static int elf_resolve_sections(struct xhot_patch_elf *elf, const void *data)
{
    struct xhot_patch_elf_sec *sec;
    unsigned int i;
    Elf_Off delta;
    int rc;

    /* xhot_patch_elf_load sanity checked e_shnum. */
    sec = xmalloc_array(struct xhot_patch_elf_sec, elf->hdr->e_shnum);
    if ( !sec )
    {
        dprintk(XENLOG_ERR, XHOT_PATCH"%s: Could not allocate memory for section table!\n",
               elf->name);
        return -ENOMEM;
    }
    ...
    /* e_shoff and e_shnum overflow checks are done in xhot_patch_header_check. */
    ...
    for ( i = 1; i < elf->hdr->e_shnum; i++ )
    {
        ...
        {
            dprintk(XENLOG_ERR, XHOT_PATCH "%s: Section [%u] data %s of payload!\n",
                    elf->name, i,
        ...
        if ( sec[i].sec->sh_type == SHT_SYMTAB )
        {
            if ( elf->symtab )
            {
                dprintk(XENLOG_ERR, XHOT_PATCH "%s: Unsupported multiple symbol tables!\n",
                        elf->name);
                return -EOPNOTSUPP;
            }
    ...
    if ( !elf->symtab )
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: No symbol table found!\n",
                elf->name);
        return -EINVAL;
    }
    ...
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Symbol table header is corrupted!\n",
                elf->name);
        return -EINVAL;
    }
    ...
    if ( rc )
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: String table section is corrupted\n",
                elf->name);
    }
...
static int elf_resolve_section_names(struct xhot_patch_elf *elf,
                                     const void *data)
{
    const char *shstrtab;
    unsigned int i;
    ...
    if ( rc )
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Section string table is corrupted\n",
                elf->name);
        return rc;
    }
    ...
    for ( i = 1; i < elf->hdr->e_shnum; i++ )
    {
        ...
        {
            dprintk(XENLOG_ERR, XHOT_PATCH "%s: Section %u name is not within .shstrtab!\n",
                    elf->name, i);
            return -EINVAL;
        }
...
static int elf_get_sym(struct xhot_patch_elf *elf, const void *data)
{
    const struct xhot_patch_elf_sec *symtab_sec, *strtab_sec;
    struct xhot_patch_elf_sym *sym;
    unsigned int i, nsym;
    Elf_Off offset;
    Elf_Word delta;
    ...
    sym = xmalloc_array(struct xhot_patch_elf_sym, nsym);
    if ( !sym )
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Could not allocate memory for symbols\n",
               elf->name);
        return -ENOMEM;
    }
    ...
    for ( i = 1; i < nsym; i++ )
    {
        ...
        {
            dprintk(XENLOG_ERR, XHOT_PATCH "%s: Symbol [%u] name is not within .strtab!\n",
                    elf->name, i);
            return -EINVAL;
        }
...
int xhot_patch_elf_resolve_symbols(struct xhot_patch_elf *elf)
{
    unsigned int i;
    int rc = 0;
    ...
    for ( i = 1; i < elf->nsym; i++ )
    {
        ...
        {
        case SHN_COMMON:
            dprintk(XENLOG_ERR, XHOT_PATCH "%s: Unexpected common symbol: %s\n",
        ...
        case SHN_UNDEF:
            ...
            if ( !st_value )
            {
                st_value = xhot_patch_symbols_lookup_by_name(elf->sym[i].name);
                if ( !st_value )
                {
                    dprintk(XENLOG_ERR, XHOT_PATCH "%s: Unknown symbol: %s\n",
                            elf->name, elf->sym[i].name);
                    rc = -ENOENT;
                    break;
                }
            }
            dprintk(XENLOG_DEBUG, XHOT_PATCH "%s: Undefined symbol resolved: %s => %#"PRIxElfAddr"\n",
                    elf->name, elf->sym[i].name, st_value);
            break;

        case SHN_ABS:
            dprintk(XENLOG_DEBUG, XHOT_PATCH "%s: Absolute symbol: %s => %#"PRIxElfAddr"\n",
        ...
        default:
            ...
            if ( rc )
            {
                dprintk(XENLOG_ERR, XHOT_PATCH "%s: Out of bounds symbol section %#x\n",
                        elf->name, idx);
                break;
            }
            ...
            if ( elf->sym[i].name )
                dprintk(XENLOG_DEBUG, XHOT_PATCH "%s: Symbol resolved: %s => %#"PRIxElfAddr" (%s)\n",
                       elf->name, elf->sym[i].name,
...
int xhot_patch_elf_perform_relocs(struct xhot_patch_elf *elf)
{
    struct xhot_patch_elf_sec *r, *base;
    unsigned int i;
    int rc = 0;
    ...
    for ( i = 1; i < elf->hdr->e_shnum; i++ )
    {
        ...
        {
            dprintk(XENLOG_ERR, XHOT_PATCH "%s: Relative link of %s is incorrect (%d, expected=%d)\n",
        ...
        if ( r->sec->sh_type == SHT_RELA )
            rc = arch_xhot_patch_perform_rela(elf, base, r);
        else /* SHT_REL */
            rc = arch_xhot_patch_perform_rel(elf, base, r);
...
static int xhot_patch_header_check(const struct xhot_patch_elf *elf)
{
    ...
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Section header is bigger than payload!\n",
                elf->name);
        return -EINVAL;
    }
    ...
    if ( !IS_ELF(*hdr) )
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Not an ELF payload!\n", elf->name);
        return -EINVAL;
    }
    ...
    if ( hdr->e_version != EV_CURRENT ||
         ...
         hdr->e_phnum != 0 )
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Invalid ELF payload!\n", elf->name);
        return -EOPNOTSUPP;
    }

    rc = arch_xhot_patch_verify_elf(elf);
    if ( rc )
        return rc;

    if ( elf->hdr->e_shstrndx == SHN_UNDEF )
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Section name idx is undefined!?\n",
                elf->name);
        return -EINVAL;
    }
    ...
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Too many (%u) sections!\n",
    ...
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Section name idx (%u) is past end of sections (%u)!\n",
    ...
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Bogus e_shoff!\n", elf->name);
        return -EINVAL;
    }

    if ( elf->hdr->e_shentsize < sizeof(Elf_Shdr) )
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Section header size is %u! Expected %zu!?\n",
    ...
    if ( ((elf->len - elf->hdr->e_shoff) / elf->hdr->e_shentsize) <
         elf->hdr->e_shnum )
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Section header size is corrupted!\n",
                elf->name);
        return -EINVAL;
    }
...
int xhot_patch_elf_load(struct xhot_patch_elf *elf, const void *data)
{
    int rc;

    elf->hdr = data;

    rc = xhot_patch_header_check(elf);
    if ( rc )
        return rc;
...
void xhot_patch_elf_free(struct xhot_patch_elf *elf)
{
    xfree(elf->sec);
    elf->sec = NULL;
    xfree(elf->sym);

...
#include <xen/xhot_patch_patch.h>
...
struct xhot_patch_build_id {
   const void *p;
   unsigned int len;
};

struct payload {
    uint32_t state;                      /* One of the XHOT_PATCH_STATE_*. */
    ...
    struct xhot_patch_patch_func *funcs;    /* The array of functions to patch. */
    unsigned int nfuncs;                 /* Nr of functions to patch. */
    const struct xhot_patch_symbol *symtab; /* All symbols. */
    const char *strtab;                  /* Pointer to .strtab. */
    struct virtual_region region;        /* symbol, bug.frame patching and
                                            exception table (x86). */
    unsigned int nsyms;                  /* Nr of entries in .strtab and symbols. */
    struct xhot_patch_build_id id;          /* ELFNOTE_DESC(.note.gnu.build-id) of the payload. */
    struct xhot_patch_build_id dep;         /* ELFNOTE_DESC(.xhot_patch.depends). */
    xhot_patch_loadcall_t **load_funcs;     /* The array of funcs to call after */
    xhot_patch_unloadcall_t **unload_funcs; /* load and unload of the payload. */
    ...
    char name[XEN_XHOT_PATCH_NAME_SIZE];    /* Name of it. */
};

/* Defines an outstanding patching action. */
struct xhot_patch_work
{
    atomic_t semaphore;          /* Used to rendezvous CPUs in
                                    check_for_xhot_patch_work. */
    ...
    unsigned int cmd;            /* Action request: XHOT_PATCH_ACTION_* */
};

/* There can be only one outstanding patching action. */
static struct xhot_patch_work xhot_patch_work;

/*
 * Indicate whether the CPU needs to consult xhot_patch_work structure.
 * We want an per-cpu data structure otherwise the check_for_xhot_patch_work
 * would hammer a global xhot_patch_work structure on every guest VMEXIT.
 * Having an per-cpu lessens the load.
 */
static DEFINE_PER_CPU(bool_t, work_to_do);

static int get_name(const xen_xhot_patch_name_t *name, char *n)
{
    if ( !name->size || name->size > XEN_XHOT_PATCH_NAME_SIZE )
        return -EINVAL;
...
static int verify_payload(const xen_sysctl_xhot_patch_upload_t *upload, char *n)
{
...
static const char *xhot_patch_symbols_lookup(unsigned long addr,
...
/*
 * Functions related to XEN_SYSCTL_XHOT_PATCH_UPLOAD (see xhot_patch_upload), and
 * freeing payload (XEN_SYSCTL_XHOT_PATCH_ACTION:XHOT_PATCH_ACTION_UNLOAD).
 */
...
*/
static void calc_section(const struct xhot_patch_elf_sec *sec, size_t *size,
                         unsigned int *offset)
{
...
static int move_payload(struct payload *payload, struct xhot_patch_elf *elf)
{
    ...
    for ( i = 1; i < elf->hdr->e_shnum; i++ )
    {
        ...
        {
            dprintk(XENLOG_DEBUG, XHOT_PATCH "%s: Not supporting %s section!\n",
    ...
    if ( !text_buf )
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Could not allocate memory for payload!\n",
                elf->name);
        rc = -ENOMEM;
        goto out;
    }
    ...
    for ( i = 1; i < elf->hdr->e_shnum; i++ )
    {
        if ( elf->sec[i].sec->sh_flags & SHF_ALLOC )
        {
            ...
            if ( elf->sec[i].sec->sh_type != SHT_NOBITS )
            {
                ...
                dprintk(XENLOG_DEBUG, XHOT_PATCH "%s: Loaded %s at %p\n",
...
static int secure_payload(struct payload *payload, struct xhot_patch_elf *elf)
{
    int rc;
    ...
    rc = arch_xhot_patch_secure(payload->text_addr, text_pages, XHOT_PATCH_VA_RX);
    if ( rc )
        return rc;
    ...
    if ( rw_pages )
    {
        rc = arch_xhot_patch_secure(payload->rw_addr, rw_pages, XHOT_PATCH_VA_RW);
        if ( rc )
            return rc;
    }
    ...
    if ( ro_pages )
        rc = arch_xhot_patch_secure(payload->ro_addr, ro_pages, XHOT_PATCH_VA_RO);
...
static int check_special_sections(const struct xhot_patch_elf *elf)
{
    unsigned int i;
    static const char *const names[] = { ELF_XHOT_PATCH_FUNC,
                                         ELF_XHOT_PATCH_DEPENDS,
                                         ELF_BUILD_ID_NOTE};
    DECLARE_BITMAP(found, ARRAY_SIZE(names)) = { 0 };

    for ( i = 0; i < ARRAY_SIZE(names); i++ )
    {
        const struct xhot_patch_elf_sec *sec;

        sec = xhot_patch_elf_sec_by_name(elf, names[i]);
        if ( !sec )
        {
            dprintk(XENLOG_ERR, XHOT_PATCH "%s: %s is missing!\n",
                    elf->name, names[i]);
            return -EINVAL;
        }
        ...
        if ( !sec->sec->sh_size )
        {
            dprintk(XENLOG_ERR, XHOT_PATCH "%s: %s is empty!\n",
                    elf->name, names[i]);
            return -EINVAL;
        }
        ...
        {
            dprintk(XENLOG_ERR, XHOT_PATCH "%s: %s was seen more than once!\n",
                    elf->name, names[i]);
            return -EINVAL;
        }
    }
...
static int prepare_payload(struct payload *payload,
                           struct xhot_patch_elf *elf)
{
    const struct xhot_patch_elf_sec *sec;
    unsigned int i;
    struct xhot_patch_patch_func *f;
    struct virtual_region *region;
    const Elf_Note *n;

    sec = xhot_patch_elf_sec_by_name(elf, ELF_XHOT_PATCH_FUNC);
    ASSERT(sec);
    if ( sec->sec->sh_size % sizeof(*payload->funcs) )
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Wrong size of "ELF_XHOT_PATCH_FUNC"!\n",
                elf->name);
        return -EINVAL;
    }
    ...
    for ( i = 0; i < payload->nfuncs; i++ )
    {
        ...
        if ( f->version != XHOT_PATCH_PAYLOAD_VERSION )
        {
            dprintk(XENLOG_ERR, XHOT_PATCH "%s: Wrong version (%u). Expected %d!\n",
                    elf->name, f->version, XHOT_PATCH_PAYLOAD_VERSION);
            return -EOPNOTSUPP;
        }

        if ( !f->new_addr || !f->new_size )
        {
            dprintk(XENLOG_ERR, XHOT_PATCH "%s: Address or size fields are zero!\n",
                    elf->name);
            return -EINVAL;
        }

        rc = arch_xhot_patch_verify_func(f);
        if ( rc )
            return rc;
        ...
        if ( !f->old_addr )
        {
            ...
            if ( !f->old_addr )
            {
                f->old_addr = (void *)xhot_patch_symbols_lookup_by_name(f->name);
                if ( !f->old_addr )
                {
                    dprintk(XENLOG_ERR, XHOT_PATCH "%s: Could not resolve old address of %s\n",
                            elf->name, f->name);
                    return -ENOENT;
                }
            }
            dprintk(XENLOG_DEBUG, XHOT_PATCH "%s: Resolved old address %s => %p\n",
                    elf->name, f->name, f->old_addr);
        }
    }

    sec = xhot_patch_elf_sec_by_name(elf, ".xhot_patch.hooks.load");
    if ( sec )
    {
        if ( !sec->sec->sh_size ||
    ...
    sec = xhot_patch_elf_sec_by_name(elf, ".xhot_patch.hooks.unload");
    if ( sec )
    {
        if ( !sec->sec->sh_size ||
    ...
    sec = xhot_patch_elf_sec_by_name(elf, ELF_BUILD_ID_NOTE);
    if ( sec )
    {
        const struct payload *data;
        ...
        list_for_each_entry ( data, &payload_list, list )
        {
            ...
            {
                dprintk(XENLOG_DEBUG, XHOT_PATCH "%s: Already loaded as %s!\n",
                        elf->name, data->name);
                return -EEXIST;
            }
        }
    ...
    sec = xhot_patch_elf_sec_by_name(elf, ELF_XHOT_PATCH_DEPENDS);
    if ( sec )
    {
        n = sec->load_addr;
    ...
    region->symbols_lookup = xhot_patch_symbols_lookup;
    ...
    for ( i = 0; i < BUGFRAME_NR; i++ )
    {
        ...
        sec = xhot_patch_elf_sec_by_name(elf, str);
        if ( !sec )
            continue;

        if ( sec->sec->sh_size % sizeof(*region->frame[i].bugs) )
        {
            dprintk(XENLOG_ERR, XHOT_PATCH "%s: Wrong size of .bug_frames.%u!\n",
                    elf->name, i);
            return -EINVAL;
        }
...
#ifndef CONFIG_ARM
    sec = xhot_patch_elf_sec_by_name(elf, ".altinstructions");
    if ( sec )
    {
        ...
        {
            dprintk(XENLOG_ERR, XHOT_PATCH "%s: Size of .alt_instr is not multiple of %zu!\n",
                    elf->name, sizeof(*a));
            return -EINVAL;
        }
        ...
        for ( a = start; a < end; a++ )
        {
            ...
            {
                dprintk(XENLOG_ERR, XHOT_PATCH "%s Alt patching outside payload: %p!\n",
                        elf->name, instr);
                return -EINVAL;
            }
        }
    ...
    sec = xhot_patch_elf_sec_by_name(elf, ".ex_table");
    if ( sec )
    {
        ...
        {
            dprintk(XENLOG_ERR, XHOT_PATCH "%s: Wrong size of .ex_table (exp:%lu vs %lu)!\n",
...
static bool_t is_payload_symbol(const struct xhot_patch_elf *elf,
                                const struct xhot_patch_elf_sym *sym)
{
...
static int build_symbol_table(struct payload *payload,
                              const struct xhot_patch_elf *elf)
{
    unsigned int i, j, nsyms = 0;
    size_t strtab_len = 0;
    struct xhot_patch_symbol *symtab;
    char *strtab;
    ...
    symtab = xmalloc_array(struct xhot_patch_symbol, nsyms);
    ...
    for ( i = 0; i < nsyms; i++ )
    {
        ...
        if ( !found )
        {
            if ( symbols_lookup_by_name(symtab[i].name) ||
                 xhot_patch_symbols_lookup_by_name(symtab[i].name) )
            {
                dprintk(XENLOG_ERR, XHOT_PATCH "%s: duplicate new symbol: %s\n",
            ...
            dprintk(XENLOG_DEBUG, XHOT_PATCH "%s: new symbol %s\n",
                     elf->name, symtab[i].name);
        }
        else
        {
            /* new_symbol is not set. */
            dprintk(XENLOG_DEBUG, XHOT_PATCH "%s: overriding symbol %s\n",
                    elf->name, symtab[i].name);
        }
    }
...
{
    struct xhot_patch_elf elf = { .name = payload->name, .len = len };
    int rc = 0;

    rc = xhot_patch_elf_load(&elf, raw);
    if ( rc )
        goto out;
    ...
    rc = xhot_patch_elf_resolve_symbols(&elf);
    if ( rc )
        goto out;

    rc = xhot_patch_elf_perform_relocs(&elf);
    if ( rc )
        goto out;
 ...
 out:
    ...
    xhot_patch_elf_free(&elf);

    return rc;
}

static int xhot_patch_upload(xen_sysctl_xhot_patch_upload_t *upload)
{
    struct payload *data, *found;
    char n[XEN_XHOT_PATCH_NAME_SIZE];
    void *raw_data;
    int rc;
    ...
    else
    {
        ...
        data->state = XHOT_PATCH_STATE_CHECKED;
...
static int xhot_patch_get(xen_sysctl_xhot_patch_get_t *get)
{
    struct payload *data;
    int rc;
    char n[XEN_XHOT_PATCH_NAME_SIZE];
...
static int xhot_patch_list(xen_sysctl_xhot_patch_list_t *list)
{
    xen_xhot_patch_status_t status;
    struct payload *data;
    ...
    if ( list->nr &&
         (!guest_handle_okay(list->status, list->nr) ||
          !guest_handle_okay(list->name, XEN_XHOT_PATCH_NAME_SIZE * list->nr) ||
    ...
    if ( list->nr )
    {
        list_for_each_entry( data, &payload_list, list )
        {
            ...
            /* N.B. 'idx' != 'i'. */
            if ( __copy_to_guest_offset(list->name, idx * XEN_XHOT_PATCH_NAME_SIZE,
                                        data->name, len) ||
 ...
 * for XEN_SYSCTL_XHOT_PATCH_ACTION operation (see xhot_patch_action).
 */
...
static int apply_payload(struct payload *data)
{
    ...
    printk(XENLOG_INFO XHOT_PATCH "%s: Applying %u functions\n",
            data->name, data->nfuncs);

    arch_xhot_patch_patching_enter();
    ...
    for ( i = 0; i < data->nfuncs; i++ )
        arch_xhot_patch_apply_jmp(&data->funcs[i]);

    arch_xhot_patch_patching_leave();
...
static int revert_payload(struct payload *data)
{
    ...
    printk(XENLOG_INFO XHOT_PATCH "%s: Reverting\n", data->name);

    arch_xhot_patch_patching_enter();

    for ( i = 0; i < data->nfuncs; i++ )
        arch_xhot_patch_revert_jmp(&data->funcs[i]);
 ...
 */
static void xhot_patch_do_action(void)
{
    int rc;
    struct payload *data, *other, *tmp;

    data = xhot_patch_work.data;
    /*
     ...
     */
    switch ( xhot_patch_work.cmd )
    {
    case XHOT_PATCH_ACTION_APPLY:
        rc = apply_payload(data);
        if ( rc == 0 )
            data->state = XHOT_PATCH_STATE_APPLIED;
        break;

    case XHOT_PATCH_ACTION_REVERT:
        rc = revert_payload(data);
        if ( rc == 0 )
            data->state = XHOT_PATCH_STATE_CHECKED;
        break;

    case XHOT_PATCH_ACTION_REPLACE:
        rc = 0;
        /*
	 ...
	 */
        list_for_each_entry_safe_reverse ( other, tmp, &applied_list, applied_list )
        {
            ...
            if ( other->rc == 0 )
                other->state = XHOT_PATCH_STATE_CHECKED;
            else
            {
                rc = -EINVAL;
                break;
    ...
    /* We must set rc as xhot_patch_action sets it to -EAGAIN when kicking of. */
    data->rc = rc;
}
...
static bool_t is_work_scheduled(const struct payload *data)
{
    ...
    return xhot_patch_work.do_work && xhot_patch_work.data == data;
}
...
static int schedule_work(struct payload *data, uint32_t cmd, uint32_t timeout)
{
    ...
    if ( xhot_patch_work.do_work )
        return -EBUSY;

    if ( !get_cpu_maps() )
    {
        printk(XENLOG_ERR XHOT_PATCH "%s: unable to get cpu_maps lock!\n",
               data->name);
        return -EBUSY;
    }

    xhot_patch_work.cmd = cmd;
    xhot_patch_work.data = data;
    xhot_patch_work.timeout = timeout ?: MILLISECS(30);

    dprintk(XENLOG_DEBUG, XHOT_PATCH "%s: timeout is %"PRI_stime"ms\n",
            data->name, xhot_patch_work.timeout / MILLISECS(1));

    atomic_set(&xhot_patch_work.semaphore, -1);

    xhot_patch_work.ready = 0;

    smp_wmb();

    xhot_patch_work.do_work = 1;
...
static int xhot_patch_spin(atomic_t *counter, s_time_t timeout,
                           unsigned int cpus, const char *s)
{
    ...
    {
        printk(XENLOG_ERR XHOT_PATCH "%s: Timed out on semaphore in %s quiesce phase %u/%u\n",
               xhot_patch_work.data->name, s, atomic_read(counter), cpus);
        rc = -EBUSY;
        xhot_patch_work.data->rc = rc;
        smp_wmb();
        xhot_patch_work.do_work = 0;
    }
...
{
#define ACTION(x) [XHOT_PATCH_ACTION_##x] = #x
...
#undef ACTION
    ...
    if ( atomic_inc_and_test(&xhot_patch_work.semaphore) )
    {
        struct payload *p;
        unsigned int cpus;

        p = xhot_patch_work.data;
        if ( !get_cpu_maps() )
        {
            printk(XENLOG_ERR XHOT_PATCH "%s: CPU%u - unable to get cpu_maps lock!\n",
                   p->name, cpu);
            per_cpu(work_to_do, cpu) = 0;
            xhot_patch_work.data->rc = -EBUSY;
            smp_wmb();
            xhot_patch_work.do_work = 0;
            /*
             * Do NOT decrement xhot_patch_work.semaphore down - as that may cause
        ...
        if ( cpus )
        {
            dprintk(XENLOG_DEBUG, XHOT_PATCH "%s: CPU%u - IPIing the other %u CPUs\n",
                    p->name, cpu, cpus);
            smp_call_function(reschedule_fn, NULL, 0);
        }

        timeout = xhot_patch_work.timeout + NOW();
        if ( xhot_patch_spin(&xhot_patch_work.semaphore, timeout, cpus, "CPU") )
            goto abort;

        /* All CPUs are waiting, now signal to disable IRQs. */
        atomic_set(&xhot_patch_work.semaphore, 0);
        /*
         ...
         */
        smp_wmb();
        xhot_patch_work.ready = 1;

        if ( !xhot_patch_spin(&xhot_patch_work.semaphore, timeout, cpus, "IRQ") )
        {
            local_irq_save(flags);
            /* Do the patching. */
            xhot_patch_do_action();
            /* Serialize and flush out the CPU via CPUID instruction (on x86). */
            arch_xhot_patch_post_action();
            local_irq_restore(flags);
        }

 abort:
        arch_xhot_patch_unmask();
        ...
        printk(XENLOG_INFO XHOT_PATCH "%s finished %s with rc=%d\n",
               p->name, names[xhot_patch_work.cmd], p->rc);
    }
    else
    {
        /* Wait for all CPUs to rendezvous. */
        while ( xhot_patch_work.do_work && !xhot_patch_work.ready )
            cpu_relax();
         ...
         */
        atomic_inc(&xhot_patch_work.semaphore);

        /* Wait for patching to complete. */
        while ( xhot_patch_work.do_work )
            cpu_relax();
...
static int xhot_patch_action(xen_sysctl_xhot_patch_action_t *action)
{
    struct payload *data;
    char n[XEN_XHOT_PATCH_NAME_SIZE];
    int rc;
    ...
    switch ( action->cmd )
    {
    case XHOT_PATCH_ACTION_UNLOAD:
        if ( data->state == XHOT_PATCH_STATE_CHECKED )
        {
            free_payload(data);
    ...
    case XHOT_PATCH_ACTION_REVERT:
        if ( data->state == XHOT_PATCH_STATE_APPLIED )
        {
...
int xhot_patch_op(xen_sysctl_xhot_patch_op_t *xhot_patch)
{
    int rc;
    ...
    switch ( xhot_patch->cmd )
    {
    case XEN_SYSCTL_XHOT_PATCH_UPLOAD:
        rc = xhot_patch_upload(&xhot_patch->u.upload);
        break;

    case XEN_SYSCTL_XHOT_PATCH_GET:
        rc = xhot_patch_get(&xhot_patch->u.get);
        break;

    case XEN_SYSCTL_XHOT_PATCH_LIST:
        rc = xhot_patch_list(&xhot_patch->u.list);
        break;

    case XEN_SYSCTL_XHOT_PATCH_ACTION:
        rc = xhot_patch_action(&xhot_patch->u.action);
        break;
...
{
#define STATE(x) [XHOT_PATCH_STATE_##x] = #x
    static const char *const names[] = {
            STATE(CHECKED),
            STATE(APPLIED),
    };
...
static void xhot_patch_printall(unsigned char key)
{
    struct payload *data;
    ...
    printk("'%c' pressed - Dumping all xhot_patch patches\n", key);
    ...
    list_for_each_entry ( data, &payload_list, list )
    {
        ...
        for ( i = 0; i < data->nfuncs; i++ )
        {
            struct xhot_patch_patch_func *f = &(data->funcs[i]);
...
static int __init xhot_patch_init(void)
{
    const void *binary_id;
    unsigned int len;

    if ( !xen_build_id(&binary_id, &len) )
        printk(XENLOG_INFO XHOT_PATCH ": build-id: %*phN\n", len, binary_id);

    register_keyhandler('x', xhot_patch_printall, "print xsplicing info", 1);

    arch_xhot_patch_init();
    return 0;
}
__initcall(xhot_patch_init);

...
static int elf_verify_strtab(const struct live_patch_elf_sec *sec)
{
    const Elf_Shdr *s;
    const char *contents;
...
static int elf_resolve_sections(struct live_patch_elf *elf, const void *data)
{
    struct live_patch_elf_sec *sec;
    unsigned int i;
    Elf_Off delta;
    int rc;

    /* live_patch_elf_load sanity checked e_shnum. */
    sec = xmalloc_array(struct live_patch_elf_sec, elf->hdr->e_shnum);
    if ( !sec )
    {
        dprintk(XENLOG_ERR, LIVE_PATCH"%s: Could not allocate memory for section table!\n",
               elf->name);
        return -ENOMEM;
    }
    ...
    /* e_shoff and e_shnum overflow checks are done in live_patch_header_check. */
    ...
    for ( i = 1; i < elf->hdr->e_shnum; i++ )
    {
        ...
        {
            dprintk(XENLOG_ERR, LIVE_PATCH "%s: Section [%u] data %s of payload!\n",
                    elf->name, i,
        ...
        if ( sec[i].sec->sh_type == SHT_SYMTAB )
        {
            if ( elf->symtab )
            {
                dprintk(XENLOG_ERR, LIVE_PATCH "%s: Unsupported multiple symbol tables!\n",
                        elf->name);
                return -EOPNOTSUPP;
            }
    ...
    {
        dprintk(XENLOG_ERR, LIVE_PATCH "%s: Symbol table header is corrupted!\n",
                elf->name);
        return -EINVAL;
    }
    ...
    if ( rc )
    {
        dprintk(XENLOG_ERR, LIVE_PATCH "%s: String table section is corrupted\n",
                elf->name);
    }
...
static int elf_resolve_section_names(struct live_patch_elf *elf, const void *data)
{
    const char *shstrtab;
    unsigned int i;
    ...
    if ( rc )
    {
        dprintk(XENLOG_ERR, LIVE_PATCH "%s: Section string table is corrupted\n",
                elf->name);
        return rc;
    }
    ...
    for ( i = 1; i < elf->hdr->e_shnum; i++ )
    {
        ...
        {
            dprintk(XENLOG_ERR, LIVE_PATCH "%s: Section %u name is not within .shstrtab!\n",
                    elf->name, i);
            return -EINVAL;
        }
...
static int elf_get_sym(struct live_patch_elf *elf, const void *data)
{
    const struct live_patch_elf_sec *symtab_sec, *strtab_sec;
    struct live_patch_elf_sym *sym;
    unsigned int i, nsym;
    Elf_Off offset;
    Elf_Word delta;
    ...
    sym = xmalloc_array(struct live_patch_elf_sym, nsym);
    if ( !sym )
    {
        dprintk(XENLOG_ERR, LIVE_PATCH "%s: Could not allocate memory for symbols\n",
               elf->name);
        return -ENOMEM;
    }
    ...
    for ( i = 1; i < nsym; i++ )
    {
        ...
        {
            dprintk(XENLOG_ERR, LIVE_PATCH "%s: Symbol [%u] name is not within .strtab!\n",
                    elf->name, i);
            return -EINVAL;
        }
...
int live_patch_elf_resolve_symbols(struct live_patch_elf *elf)
{
    ...
    for ( i = 1; i < elf->nsym; i++ )
    {
        ...
        {
        case SHN_COMMON:
            dprintk(XENLOG_ERR, LIVE_PATCH "%s: Unexpected common symbol: %s\n",
        ...
        case SHN_UNDEF:
            ...
            if ( !st_value )
            {
                st_value = live_patch_symbols_lookup_by_name(elf->sym[i].name);
                if ( !st_value )
                {
            ...
            dprintk(XENLOG_DEBUG, LIVE_PATCH "%s: Undefined symbol resolved: %s => %#"PRIxElfAddr"\n",
                    elf->name, elf->sym[i].name, st_value);
            break;

        case SHN_ABS:
            dprintk(XENLOG_DEBUG, LIVE_PATCH "%s: Absolute symbol: %s => %#"PRIxElfAddr"\n",
        ...
        default:
            ...
            if ( rc )
            {
                dprintk(XENLOG_ERR, LIVE_PATCH "%s: Out of bounds symbol section %#x\n",
                        elf->name, idx);
                break;
            }
            ...
            if ( elf->sym[i].name )
                dprintk(XENLOG_DEBUG, LIVE_PATCH "%s: Symbol resolved: %s => %#"PRIxElfAddr" (%s)\n",
                       elf->name, elf->sym[i].name,
...
{
    struct live_patch_elf_sec *r, *base;
    unsigned int i;
    int rc = 0;
    ...
    for ( i = 1; i < elf->hdr->e_shnum; i++ )
    {
        ...
        {
            dprintk(XENLOG_ERR, LIVE_PATCH "%s: Relative link of %s is incorrect (%d, expected=%d)\n",
        ...
        if ( r->sec->sh_type == SHT_RELA )
            rc = arch_live_patch_perform_rela(elf, base, r);
        else /* SHT_REL */
            rc = arch_live_patch_perform_rel(elf, base, r);
...
static int live_patch_header_check(const struct live_patch_elf *elf)
{
    ...
    {
        dprintk(XENLOG_ERR, LIVE_PATCH "%s: Section header is bigger than payload!\n",
                elf->name);
        return -EINVAL;
    }
    ...
    rc = arch_live_patch_verify_elf(elf);
    if ( rc )
        return rc;
    ...
    {
        dprintk(XENLOG_ERR, LIVE_PATCH "%s: Section name idx (%u) is past end of sections (%u)!\n",
    ...
    {
        dprintk(XENLOG_ERR, LIVE_PATCH "%s: Section header size is %u! Expected %zu!?\n",
    ...
    if ( ((elf->len - elf->hdr->e_shoff) / elf->hdr->e_shentsize) <
         elf->hdr->e_shnum )
    {
        dprintk(XENLOG_ERR, LIVE_PATCH "%s: Section header size is corrupted!\n",
                elf->name);
        return -EINVAL;
    }
...
int live_patch_elf_load(struct live_patch_elf *elf, const void *data)
{
    ...
    rc = live_patch_header_check(elf);
    if ( rc )
        return rc;

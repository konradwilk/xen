                                ...
                                const struct xsplice_elf_sym *sym)
{
     ...
     *  1) For SHF_ALLOC - ignore symbols referring to sections that are not
     *    loaded.
     *  2) For SHF_MERGE - ignore local symbols referring to mergeable sections.
     *    (ld squashes them all in one section and discards the symbols).
     *  3) .LCx - those are  intermediate artifacts of assembly which are
     *     even more local. Same behavior as 2).
     *
     * See elf_link_input_bfd and _bfd_elf_is_local_label_name in binutils.
     */
    return (elf->sec[sym->sym->st_shndx].sec->sh_flags & SHF_ALLOC) && /* 1*/
           !((elf->sec[sym->sym->st_shndx].sec->sh_flags & SHF_MERGE) && /* 2 */
             !strncmp(sym->name, ".L", 2)); /* 3 */
}

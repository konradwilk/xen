...
struct payload {
    ...
    struct live_patch_func *funcs;       /* The array of functions to patch. */
    unsigned int nfuncs;                 /* Nr of functions to patch. */
    const struct live_patch_symbol *symtab; /* All symbols. */
    const char *strtab;                  /* Pointer to .strtab. */
    struct virtual_region region;        /* symbol, bug.frame patching and
                                            exception table (x86). */
    unsigned int nsyms;                  /* Nr of entries in .strtab and symbols. */
    struct live_patch_build_id id;       /* ELFNOTE_DESC(.note.gnu.build-id) of the payload. */
    struct live_patch_build_id dep;      /* ELFNOTE_DESC(.live_patch.depends). */
    char name[XEN_LIVE_PATCH_NAME_SIZE]; /* Name of it. */
};

/* Defines an outstanding patching action. */
struct live_patch_work
{
...
static struct live_patch_work live_patch_work;

/*
 * Indicate whether the CPU needs to consult live_patch_work structure.
 * We want an per-cpu data structure otherwise the check_for_live_patch_work
 * would hammer a global live_patch_work structure on every guest VMEXIT.
 * Having an per-cpu lessens the load.
 */
static DEFINE_PER_CPU(bool_t, work_to_do);

static int get_name(const xen_live_patch_name_t *name, char *n)
{
...
static int verify_payload(const xen_sysctl_live_patch_upload_t *upload, char *n)
{
...
static const char *live_patch_symbols_lookup(unsigned long addr,
...
/*
 * Functions related to XEN_SYSCTL_LIVE_PATCH_UPLOAD (see live_patch_upload), and
...
*/
static void calc_section(const struct live_patch_elf_sec *sec, size_t *size,
                         unsigned int *offset)
{
...
static int move_payload(struct payload *payload, struct live_patch_elf *elf)
{
    ...
    for ( i = 1; i < elf->hdr->e_shnum; i++ )
    {
        ...
        {
            dprintk(XENLOG_DEBUG, LIVE_PATCH "%s: Not supporting %s section!\n",
    ...
    if ( !text_buf )
    {
        dprintk(XENLOG_ERR, LIVE_PATCH "%s: Could not allocate memory for payload!\n",
                elf->name);
        rc = -ENOMEM;
        goto out;
    }
...
static int secure_payload(struct payload *payload, struct live_patch_elf *elf)
{
    int rc;
    ...
    rc = arch_live_patch_secure(payload->text_addr, text_pages, LIVE_PATCH_VA_RX);
    if ( rc )
        return rc;
    ...
    if ( rw_pages )
    {
        rc = arch_live_patch_secure(payload->rw_addr, rw_pages, LIVE_PATCH_VA_RW);
        if ( rc )
            return rc;
    }
    ...
    if ( ro_pages )
        rc = arch_live_patch_secure(payload->ro_addr, ro_pages, LIVE_PATCH_VA_RO);
...
static int check_special_sections(const struct live_patch_elf *elf)
{
    unsigned int i;
    ...
    {
        const struct live_patch_elf_sec *sec;

        sec = live_patch_elf_sec_by_name(elf, names[i]);
        if ( !sec )
        {
...
static int prepare_payload(struct payload *payload,
                           struct live_patch_elf *elf)
{
    const struct live_patch_elf_sec *sec;
    unsigned int i;
    struct live_patch_func *f;
    struct virtual_region *region;
    const Elf_Note *n;

    sec = live_patch_elf_sec_by_name(elf, ELF_LIVE_PATCH_FUNC);
    ASSERT(sec);
    if ( sec->sec->sh_size % sizeof(*payload->funcs) )
    {
        dprintk(XENLOG_ERR, LIVE_PATCH "%s: Wrong size of "ELF_LIVE_PATCH_FUNC"!\n",
                elf->name);
        return -EINVAL;
    }
    ...
    for ( i = 0; i < payload->nfuncs; i++ )
    {
        ...
        {
            dprintk(XENLOG_ERR, LIVE_PATCH "%s: Wrong version (%u). Expected %d!\n",
        ...
        {
            dprintk(XENLOG_ERR, LIVE_PATCH "%s: Address or size fields are zero!\n",
                    elf->name);
            return -EINVAL;
        }

        rc = arch_live_patch_verify_func(f);
        if ( rc )
            return rc;
        ...
        if ( !f->old_addr )
        {
            ...
            if ( !f->old_addr )
            {
                f->old_addr = (void *)live_patch_symbols_lookup_by_name(f->name);
                if ( !f->old_addr )
                {
                    dprintk(XENLOG_ERR, LIVE_PATCH "%s: Could not resolve old address of %s\n",
                            elf->name, f->name);
                    return -ENOENT;
                }
            }
            dprintk(XENLOG_DEBUG, LIVE_PATCH "%s: Resolved old address %s => %p\n",
                    elf->name, f->name, f->old_addr);
        }
    }

    sec = live_patch_elf_sec_by_name(elf, ELF_BUILD_ID_NOTE);
    if ( sec )
    {
        const struct payload *data;
    ...
    sec = live_patch_elf_sec_by_name(elf, ELF_LIVE_PATCH_DEPENDS);
    if ( sec )
    {
        n = sec->load_addr;
    ...
    region->symbols_lookup = live_patch_symbols_lookup;
    ...
    for ( i = 0; i < BUGFRAME_NR; i++ )
    {
        ...
        sec = live_patch_elf_sec_by_name(elf, str);
        if ( !sec )
            continue;

        if ( sec->sec->sh_size % sizeof(*region->frame[i].bugs) )
        {
            dprintk(XENLOG_ERR, LIVE_PATCH "%s: Wrong size of .bug_frames.%u!\n",
                    elf->name, i);
            return -EINVAL;
        }
...
#ifndef CONFIG_ARM
    sec = live_patch_elf_sec_by_name(elf, ".altinstructions");
    if ( sec )
    {
        ...
        {
            dprintk(XENLOG_ERR, LIVE_PATCH "%s: Size of .alt_instr is not multiple of %zu!\n",
                    elf->name, sizeof(*a));
            return -EINVAL;
        }
        ...
        for ( a = start; a < end; a++ )
        {
            ...
            {
                dprintk(XENLOG_ERR, LIVE_PATCH "%s Alt patching outside payload: %p!\n",
                        elf->name, instr);
                return -EINVAL;
            }
        }
    ...
    sec = live_patch_elf_sec_by_name(elf, ".ex_table");
    if ( sec )
    {
        ...
        {
            dprintk(XENLOG_ERR, LIVE_PATCH "%s: Wrong size of .ex_table (exp:%lu vs %lu)!\n",
...
static bool_t is_payload_symbol(const struct live_patch_elf *elf,
                                const struct live_patch_elf_sym *sym)
{
...
static int build_symbol_table(struct payload *payload,
                              const struct live_patch_elf *elf)
{
    unsigned int i, j, nsyms = 0;
    size_t strtab_len = 0;
    struct live_patch_symbol *symtab;
    char *strtab;
    ...
    symtab = xmalloc_array(struct live_patch_symbol, nsyms);
    ...
    for ( i = 0; i < nsyms; i++ )
    {
        ...
        if ( !found )
        {
            if ( symbols_lookup_by_name(symtab[i].name) ||
                 live_patch_symbols_lookup_by_name(symtab[i].name) )
            {
                dprintk(XENLOG_ERR, LIVE_PATCH "%s: duplicate new symbol: %s\n",
...
{
    struct live_patch_elf elf = { .name = payload->name, .len = len };
    int rc = 0;

    rc = live_patch_elf_load(&elf, raw);
    if ( rc )
        goto out;
    ...
    rc = live_patch_elf_resolve_symbols(&elf);
    if ( rc )
        goto out;

    rc = live_patch_elf_perform_relocs(&elf);
    if ( rc )
        goto out;
 ...
 out:
    ...
    live_patch_elf_free(&elf);

    return rc;
}

static int live_patch_upload(xen_sysctl_live_patch_upload_t *upload)
{
...
static int live_patch_get(xen_sysctl_live_patch_get_t *get)
{
    struct payload *data;
    int rc;
    char n[XEN_LIVE_PATCH_NAME_SIZE];
...
static int live_patch_list(xen_sysctl_live_patch_list_t *list)
{
    xen_live_patch_status_t status;
    struct payload *data;
    ...
    if ( list->nr )
    {
        list_for_each_entry( data, &payload_list, list )
        {
            ...
            /* N.B. 'idx' != 'i'. */
            if ( __copy_to_guest_offset(list->name, idx * XEN_LIVE_PATCH_NAME_SIZE,
                                        data->name, len) ||
 ...
 * for XEN_SYSCTL_LIVE_PATCH_ACTION operation (see live_patch_action).
 */
...
static int apply_payload(struct payload *data)
{
    ...
    arch_live_patching_enter();

    for ( i = 0; i < data->nfuncs; i++ )
        arch_live_patch_apply_jmp(&data->funcs[i]);

    arch_live_patching_leave();
...
static int revert_payload(struct payload *data)
{
    ...
    for ( i = 0; i < data->nfuncs; i++ )
        arch_live_patch_revert_jmp(&data->funcs[i]);
 ...
 */
static void live_patch_do_action(void)
{
    int rc;
    struct payload *data, *other, *tmp;

    data = live_patch_work.data;
    /*
    ...
    /* We must set rc as live_patch_action sets it to -EAGAIN when kicking of. */
    data->rc = rc;
}
...
static bool_t is_work_scheduled(const struct payload *data)
{
    ...
    return live_patch_work.do_work && live_patch_work.data == data;
}
...
static int schedule_work(struct payload *data, uint32_t cmd, uint32_t timeout)
{
    ...
    live_patch_work.cmd = cmd;
    live_patch_work.data = data;
    live_patch_work.timeout = timeout ?: MILLISECS(30);

    dprintk(XENLOG_DEBUG, LIVE_PATCH "%s: timeout is %"PRI_stime"ms\n",
            data->name, live_patch_work.timeout / MILLISECS(1));

    atomic_set(&live_patch_work.semaphore, -1);

    live_patch_work.ready = 0;

    smp_wmb();

    live_patch_work.do_work = 1;
...
static int live_patch_spin(atomic_t *counter, s_time_t timeout,
                           unsigned int cpus, const char *s)
{
    ...
    {
        printk(XENLOG_ERR LIVE_PATCH "%s: Timed out on semaphore in %s quiesce phase %u/%u\n",
               live_patch_work.data->name, s, atomic_read(counter), cpus);
        rc = -EBUSY;
        live_patch_work.data->rc = rc;
        smp_wmb();
        live_patch_work.do_work = 0;
    }
...
#undef ACTION
    ...
    if ( atomic_inc_and_test(&live_patch_work.semaphore) )
    {
        struct payload *p;
        unsigned int cpus;

        p = live_patch_work.data;
        if ( !get_cpu_maps() )
        {
            printk(XENLOG_ERR LIVE_PATCH "%s: CPU%u - unable to get cpu_maps lock!\n",
                   p->name, cpu);
            per_cpu(work_to_do, cpu) = 0;
            live_patch_work.data->rc = -EBUSY;
            smp_wmb();
            live_patch_work.do_work = 0;
            /*
             * Do NOT decrement live_patch_work.semaphore down - as that may cause
        ...
        if ( cpus )
        {
            dprintk(XENLOG_DEBUG, LIVE_PATCH "%s: CPU%u - IPIing the other %u CPUs\n",
                    p->name, cpu, cpus);
            smp_call_function(reschedule_fn, NULL, 0);
        }

        timeout = live_patch_work.timeout + NOW();
        if ( live_patch_spin(&live_patch_work.semaphore, timeout, cpus, "CPU") )
            goto abort;

        /* All CPUs are waiting, now signal to disable IRQs. */
        atomic_set(&live_patch_work.semaphore, 0);
        /*
         ...
         */
        smp_wmb();
        live_patch_work.ready = 1;

        if ( !live_patch_spin(&live_patch_work.semaphore, timeout, cpus, "IRQ") )
        {
            local_irq_save(flags);
            /* Do the patching. */
            live_patch_do_action();
            /* Serialize and flush out the CPU via CPUID instruction (on x86). */
            arch_live_patch_post_action();
            local_irq_restore(flags);
        }

 abort:
        arch_live_patch_unmask();
        ...
        printk(XENLOG_INFO LIVE_PATCH "%s finished %s with rc=%d\n",
               p->name, names[live_patch_work.cmd], p->rc);
    }
    else
    {
        /* Wait for all CPUs to rendezvous. */
        while ( live_patch_work.do_work && !live_patch_work.ready )
            cpu_relax();
         ...
         */
        atomic_inc(&live_patch_work.semaphore);

        /* Wait for patching to complete. */
        while ( live_patch_work.do_work )
            cpu_relax();
...
static int live_patch_action(xen_sysctl_live_patch_action_t *action)
{
    struct payload *data;
    char n[XEN_LIVE_PATCH_NAME_SIZE];
    int rc;
...
int live_patch_op(xen_sysctl_live_patch_op_t *live_patch)
{
    int rc;
    ...
    switch ( live_patch->cmd )
    {
    case XEN_SYSCTL_LIVE_PATCH_UPLOAD:
        rc = live_patch_upload(&live_patch->u.upload);
        break;

    case XEN_SYSCTL_LIVE_PATCH_GET:
        rc = live_patch_get(&live_patch->u.get);
        break;

    case XEN_SYSCTL_LIVE_PATCH_LIST:
        rc = live_patch_list(&live_patch->u.list);
        break;

    case XEN_SYSCTL_LIVE_PATCH_ACTION:
        rc = live_patch_action(&live_patch->u.action);
        break;
...
static void live_patch_printall(unsigned char key)
{
    struct payload *data;
    ...
    printk("'%c' pressed - Dumping all live_patch patches\n", key);
    ...
    list_for_each_entry ( data, &payload_list, list )
    {
        ...
        for ( i = 0; i < data->nfuncs; i++ )
        {
            struct live_patch_func *f = &(data->funcs[i]);
...
static int __init live_patch_init(void)
{
    ...
    register_keyhandler('x', live_patch_printall, "print live_patch info", 1);

    arch_live_patch_init();
    return 0;
}
__initcall(live_patch_init);

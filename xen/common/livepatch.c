...
struct payload {
    ...
    unsigned int *bss_size;              /* and their sizes. */
                               ...
                               const struct livepatch_elf *elf)
{
    ...
    if ( plus )
    {
        ...
        /* Temporary mutiliation so that symbol lookup works. */
        *plus = '\0';
    }
    ...
    /* Lets put the '+' back in the symbol name. */
    if ( plus )
    {
        *plus = '+';
        f->old_addr += offset;
...
static int move_payload(struct payload *payload, struct livepatch_elf *elf)
{
    ...
    payload->bss_size = xmalloc_array(unsigned int, n_bss);

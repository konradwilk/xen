...
/*
 * Functions related to XEN_SYSCTL_LIVE_PATCH_UPLOAD (see livepatch_upload), and
...
static int secure_payload(struct payload *payload, struct livepatch_elf *elf)
{
    ...
    rc = arch_livepatch_secure(payload->text_addr, text_pages, LIVE_PATCH_VA_RX);
    if ( rc )
        return rc;
    ...
    if ( rw_pages )
    {
        rc = arch_livepatch_secure(payload->rw_addr, rw_pages, LIVE_PATCH_VA_RW);
        if ( rc )
            return rc;
    }
    ...
    if ( ro_pages )
        rc = arch_livepatch_secure(payload->ro_addr, ro_pages, LIVE_PATCH_VA_RO);

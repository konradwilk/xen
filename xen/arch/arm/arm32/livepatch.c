                               ...
                               const struct livepatch_elf_sec *rela)
{
    ...
    dprintk(XENLOG_INFO, LIVEPATCH "%s: base=%s, rela=%s\n", elf->name,
           base->name, rela->name);
	...
	dprintk(XENLOG_INFO, LIVEPATCH "%s, dest=0x%lx, val=0x%x, type: 0x%x\n",
		elf->sym[symndx].name, (unsigned long)dest, val, ELF32_R_TYPE(r->r_info));

	dprintk(XENLOG_INFO, LIVEPATCH "0x%x -> ", *(u32 *)dest);
	...
	dprintk(XENLOG_INFO, LIVEPATCH "%s,done  *[0x%x]\n",
		elf->sym[symndx].name, *(uint32_t *)dest);
    }
    return 0;
}
/*

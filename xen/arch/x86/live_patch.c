                              ...
                              const struct live_patch_elf_sec *rela)
{
    ...
    {
        dprintk(XENLOG_ERR, LIVE_PATCH "%s: Section relative header is corrupted!\n",
                elf->name);
        return -EINVAL;
    }
    ...
    for ( i = 0; i < (rela->sec->sh_size / rela->sec->sh_entsize); i++ )
    {
        ...
        if ( symndx > elf->nsym )
        {
            dprintk(XENLOG_ERR, LIVE_PATCH "%s: Relative relocation wants symbol@%u which is past end!\n",
                    elf->name, symndx);
            return -EINVAL;
        }
        ...
        case R_X86_64_PC32:
            ...
            *(int32_t *)dest = val;
            ...
            {
                dprintk(XENLOG_ERR, LIVE_PATCH "%s: Overflow in relocation %u in %s for %s!\n",
 ...
 bad_offset:
    dprintk(XENLOG_ERR, LIVE_PATCH "%s: Relative relocation offset is past %s section!\n",
            elf->name, base->name);
    return -EINVAL;
}

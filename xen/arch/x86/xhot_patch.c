...
int arch_xhot_patch_verify_elf(const struct xhot_patch_elf *elf)
{
    ...
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Unsupported ELF Machine type!\n",
                elf->name);
        return -EOPNOTSUPP;
    }
...
{
    dprintk(XENLOG_ERR, XHOT_PATCH "%s: SHT_REL relocation unsupported\n",
            elf->name);
    return -EOPNOTSUPP;
}
                                 ...
                                 const struct xhot_patch_elf_sec *rela)
{
    ...
    {
        dprintk(XENLOG_ERR, XHOT_PATCH "%s: Section relative header is corrupted!\n",
                elf->name);
        return -EINVAL;
    }
    ...
    for ( i = 0; i < (rela->sec->sh_size / rela->sec->sh_entsize); i++ )
    {
        ...
        if ( symndx > elf->nsym )
        {
            dprintk(XENLOG_ERR, XHOT_PATCH "%s: Relative relocation wants symbol@%u which is past end!\n",
                    elf->name, symndx);
            return -EINVAL;
        }
        ...
        case R_X86_64_PC32:
            ...
            *(int32_t *)dest = val;
            ...
            {
                dprintk(XENLOG_ERR, XHOT_PATCH "%s: Overflow in relocation %u in %s for %s!\n",
        ...
        default:
            dprintk(XENLOG_ERR, XHOT_PATCH "%s: Unhandled relocation %lu\n",
 ...
 bad_offset:
    dprintk(XENLOG_ERR, XHOT_PATCH "%s: Relative relocation offset is past %s section!\n",
            elf->name, base->name);
    return -EINVAL;
}
...
int arch_xhot_patch_secure(const void *va, unsigned int pages, enum va_type type)
{
    ...
    if ( type == XHOT_PATCH_VA_RX )
        flag = PAGE_HYPERVISOR_RX;
    else if ( type == XHOT_PATCH_VA_RW )

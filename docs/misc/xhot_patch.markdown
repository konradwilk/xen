...
The xHot Patch payload is a relocatable ELF binary. A typical binary would have:
...
The xHot Patch core code loads the payload as a standard ELF binary, relocates it
 ...
 * `.xhot_patch.funcs` - which is an array of xhot_patch_patch_func structures.
 * `.xhot_patch.depends` - which is an ELF Note that describes what the payload
    depends on. **MUST** have one.
 *  `.note.gnu.build-id` - the build-id of this payload. **MUST** have one.

### .xhot_patch.funcs

The `.xhot_patch.funcs` contains an array of xhot_patch_patch_func structures
...
The size of the `xhot_patch_patch_func` array is determined from the ELF section
size.

When applying the patch the hypervisor iterates over each `xhot_patch_patch_func`
...
When reverting a patch, the hypervisor iterates over each `xhot_patch_patch_func`
 ...
 * `.xhot_patch.hooks.load` - an array of function pointers.
 * `.xhot_patch.hooks.unload` - an array of function pointers.


### Example of .xhot_patch.funcs
...
struct xhot_patch_patch_func xhot_patch_hello_world = {  
    ...
    .old_size = 13, /* -----------""---------------  */  
} __attribute__((__section__(".xhot_patch.funcs")));  

</pre>

Code must be compiled with -fPIC.

### .xhot_patch.hooks.load and .xhot_patch.hooks.unload

This section contains an array of function pointers to be executed
before payload is being applied (.xhot_patch.funcs) or after reverting
...
<pre>
typedef void (*xhot_patch_loadcall_t)(void);  
typedef void (*xhot_patch_unloadcall_t)(void);   
</pre>

### .xhot_patch.depends and .note.gnu.build-id

To support dependencies checking and safe loading (to load the
appropiate payload against the right hypervisor) there is a need
to embbed an build-id dependency.

This is done by the payload containing an section `.xhot_patch.depends`
...
### Basic type: struct xen_xhot_patch_name

Most of the hypercalls employ an shared structure called `struct xen_xhot_patch_name`
which contains:
 ...
 * A `struct xen_xhot_patch_name` called `name` which has the unique name.
 ...
 * A `struct xen_xhot_patch_name` called `name` which has the unique name.
 * A `struct xen_xhot_patch_status` structure. The member values will
 ...
 * `status` - indicates the current status of the payload:
   * *XHOT_PATCH_STATUS_CHECKED*  (1) loaded and the ELF payload safety checks passed.
   * *XHOT_PATCH_STATUS_APPLIED* (2) loaded, checked, and applied.
   *  No other value is possible.
 * `rc` - -XEN_EXX type errors encountered while performing the last
   XHOT_PATCH_ACTION_* operation. The normal values can be zero or -XEN_EAGAIN which
...
<pre>
 status: XHOT_PATCH_STATUS_CHECKED
 rc: 0
</pre>

We apply an action - XHOT_PATCH_ACTION_REVERT - to revert it (which won't work
...
#define XHOT_PATCH_STATUS_CHECKED      1  
#define XHOT_PATCH_STATUS_APPLIED      2  
    uint32_t state;                 /* OUT: XHOT_PATCH_STATE_*. */  
    int32_t rc;                     /* OUT: 0 if no error, otherwise -XEN_EXX. */  
};  

struct xen_sysctl_xhot_patch_get {  
    xen_xhot_patch_name_t name;        /* IN, the name of the payload. */  
    xen_xhot_patch_status_t status;    /* IN/OUT: status of the payload. */  
};  
</pre>
 ...
 * `pad` - *MUST* be zero.
 * `status` virtual address of where to write `struct xen_xhot_patch_status`
 ...
 * `name` - virtual address of where to write the unique name of the payload.
   ...
   **XEN_XHOT_PATCH_NAME_SIZE** size. Note that **XEN_XHOT_PATCH_NAME_SIZE** includes
   the NUL terminator.
...
The `struct xen_xhot_patch_status` structure contains an status of payload which includes:
 ...
 * A 'struct xen_xhot_patch_name` `name` containing the unique name.
 * `cmd` the command requested:
  * *XHOT_PATCH_ACTION_UNLOAD* (1) unload the payload.
   Any further hypercalls against the `name` will result in failure unless
   **XEN_SYSCTL_XHOT_PATCH_UPLOAD** hypercall is perfomed with same `name`.
  * *XHOT_PATCH_ACTION_REVERT* (2) revert the payload. If the operation takes
  more time than the upper bound of time the `rc` in `xen_xhot_patch_status'
  retrieved via **XEN_SYSCTL_XHOT_PATCH_GET** will be -XEN_EBUSY.
  * *XHOT_PATCH_ACTION_APPLY* (3) apply the payload. If the operation takes
  more time than the upper bound of time the `rc` in `xen_xhot_patch_status'
  retrieved via **XEN_SYSCTL_XHOT_PATCH_GET** will be -XEN_EBUSY.
  * *XHOT_PATCH_ACTION_REPLACE* (4) revert all applied payloads and apply this
  payload. If the operation takes more time than the upper bound of time
  the `rc` in `xen_xhot_patch_status' retrieved via **XEN_SYSCTL_XHOT_PATCH_GET**
  will be -XEN_EBUSY.
...
## State diagrams of XHOT_PATCH_ACTION commands.

There is a strict ordering state of what the commands can be.
The XHOT_PATCH_ACTION prefix has been dropped to easy reading and
does not include the XHOT_PATCH_STATES:
...
## State transition table of XHOT_PATCH_ACTION commands and XHOT_PATCH_STATUS.

Note that:

 - The CHECKED state is the starting one achieved with *XEN_SYSCTL_XHOT_PATCH_UPLOAD* hypercall.
...
The normal sequence of events is to:
 ...
 1. *XEN_SYSCTL_XHOT_PATCH_UPLOAD* to upload the payload. If there are errors *STOP* here.
 2. *XEN_SYSCTL_XHOT_PATCH_GET* to check the `->rc`. If *-XEN_EAGAIN* spin. If zero go to next step.
 3. *XEN_SYSCTL_XHOT_PATCH_ACTION* with *XHOT_PATCH_ACTION_APPLY* to apply the patch.
 4. *XEN_SYSCTL_XHOT_PATCH_GET* to check the `->rc`. If in *-XEN_EAGAIN* spin. If zero exit with success.
...
### Generation of xHot Patch ELF payloads
...
Currently it resides at https://github.com/rosslagerwall/xhot_patch-build
...
### xHot Patch interdependencies

xHot Patch patches interdependencies are tricky.
...
This is for further development of xHot Patch.
 ...
 * Further safety checks (blacklist of which functions cannot be patched, check
   ...
   Specifically we want to make sure that xHot Patch codepaths cannot be patched.

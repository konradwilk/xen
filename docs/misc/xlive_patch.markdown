 ...
 * `.xlive_patch.funcs` - which is an array of xlive_patch_func structures.
 * `.xlive_patch.depends` - which is an ELF Note that describes what the payload
    depends on. **MUST** have one.
 *  `.note.gnu.build-id` - the build-id of this payload. **MUST** have one.

### .xlive_patch.funcs

The `.xlive_patch.funcs` contains an array of xlive_patch_func structures
which describe the functions to be patched:

<pre>
struct xlive_patch_func {  
    const char *name;  
    void *new_addr;  
    void *old_addr;  
    uint32_t new_size;  
...
The size of the `xlive_patch_func` array is determined from the ELF section
size.

When applying the patch the hypervisor iterates over each `xlive_patch_func`
...
When reverting a patch, the hypervisor iterates over each `xlive_patch_func`
 ...
 * `.xlive_patch.hooks.load` - an array of function pointers.
 * `.xlive_patch.hooks.unload` - an array of function pointers.


### Example of .xlive_patch.funcs
...
struct xlive_patch_func xlive_patch_hello_world = {  
    ...
    .old_size = 13, /* -----------""---------------  */  
} __attribute__((__section__(".xlive_patch.funcs")));  

</pre>

Code must be compiled with -fPIC.

### .xlive_patch.hooks.load and .xlive_patch.hooks.unload

This section contains an array of function pointers to be executed
before payload is being applied (.xlive_patch.funcs) or after reverting
...
<pre>
typedef void (*xlive_patch_loadcall_t)(void);  
typedef void (*xlive_patch_unloadcall_t)(void);   
</pre>

### .xlive_patch.depends and .note.gnu.build-id

To support dependencies checking and safe loading (to load the
appropiate payload against the right hypervisor) there is a need
to embbed an build-id dependency.

This is done by the payload containing an section `.xlive_patch.depends`
...
struct xen_sysctl_xlive_patch_op {  
    uint32_t cmd;                   /* IN: XEN_SYSCTL_XLIVE_PATCH_*. */  
...
### Basic type: struct xen_xlive_patch_name

Most of the hypercalls employ an shared structure called `struct xen_xlive_patch_name`
which contains:
 ...
 */  
#define XEN_XLIVE_PATCH_NAME_SIZE 128  
struct xen_xlive_patch_name {  
    XEN_GUEST_HANDLE_64(char) name;         /* IN, pointer to name. */  
    uint16_t size;                          /* IN, size of name. May be upto   
                                               XEN_XLIVE_PATCH_NAME_SIZE. */  
    uint16_t pad[3];                        /* IN: MUST be zero. */ 
};  
</pre>

### XEN_SYSCTL_XLIVE_PATCH_UPLOAD (0)

Upload a payload to the hypervisor. The payload is verified
against basic checks and if there are any issues the proper return code
will be returned. The payload is not applied at this time - that is
controlled by *XEN_SYSCTL_XLIVE_PATCH_ACTION*.

The caller provides:

 * A `struct xen_xlive_patch_name` called `name` which has the unique name.
...
<pre>
struct xen_sysctl_xlive_patch_upload {  
    xen_xlive_patch_name_t name;            /* IN, name of the patch. */  
    uint64_t size;                      /* IN, size of the ELF file. */  
    XEN_GUEST_HANDLE_64(uint8) payload; /* IN: ELF file. */  
};  
</pre>

### XEN_SYSCTL_XLIVE_PATCH_GET (1)

Retrieve an status of an specific payload. This caller provides:

 * A `struct xen_xlive_patch_name` called `name` which has the unique name.
 * A `struct xen_xlive_patch_status` structure. The member values will
   be over-written upon completion.

Upon completion the `struct xen_xlive_patch_status` is updated.

 * `status` - indicates the current status of the payload:
   * *XLIVE_PATCH_STATUS_CHECKED*  (1) loaded and the ELF payload safety checks passed.
   * *XLIVE_PATCH_STATUS_APPLIED* (2) loaded, checked, and applied.
   *  No other value is possible.
 * `rc` - -XEN_EXX type errors encountered while performing the last
   XLIVE_PATCH_ACTION_* operation. The normal values can be zero or -XEN_EAGAIN which
...
<pre>
 status: XLIVE_PATCH_STATUS_CHECKED
 rc: 0
</pre>

We apply an action - XLIVE_PATCH_ACTION_REVERT - to revert it (which won't work
...
<pre>
struct xen_xlive_patch_status {  
#define XLIVE_PATCH_STATUS_CHECKED      1  
#define XLIVE_PATCH_STATUS_APPLIED      2  
    uint32_t state;                 /* OUT: XLIVE_PATCH_STATE_*. */  
    int32_t rc;                     /* OUT: 0 if no error, otherwise -XEN_EXX. */  
};  

struct xen_sysctl_xlive_patch_get {  
    xen_xlive_patch_name_t name;        /* IN, the name of the payload. */  
    xen_xlive_patch_status_t status;    /* IN/OUT: status of the payload. */  
};  
</pre>

### XEN_SYSCTL_XLIVE_PATCH_LIST (2)
 ...
 * `pad` - *MUST* be zero.
 * `status` virtual address of where to write `struct xen_xlive_patch_status`
 ...
 * `name` - virtual address of where to write the unique name of the payload.
   ...
   **XEN_XLIVE_PATCH_NAME_SIZE** size. Note that **XEN_XLIVE_PATCH_NAME_SIZE** includes
   the NUL terminator.
...
The `struct xen_xlive_patch_status` structure contains an status of payload which includes:
...
<pre>
struct xen_sysctl_xlive_patch_list {  
    ...
    XEN_GUEST_HANDLE_64(xen_xlive_patch_status_t) status;  /* OUT. Must have enough  
                                               space allocate for nr of them. */  
    XEN_GUEST_HANDLE_64(char) id;           /* OUT: Array of names. Each member  
                                               MUST XEN_XLIVE_PATCH_NAME_SIZE in size.  
...
### XEN_SYSCTL_XLIVE_PATCH_ACTION (3)

Perform an operation on the payload structure referenced by the `name` field.
The operation request is asynchronous and the status should be retrieved
by using either **XEN_SYSCTL_XLIVE_PATCH_GET** or **XEN_SYSCTL_XLIVE_PATCH_LIST** hypercall.

The caller provides:

 * A 'struct xen_xlive_patch_name` `name` containing the unique name.
 * `cmd` the command requested:
  * *XLIVE_PATCH_ACTION_UNLOAD* (1) unload the payload.
   Any further hypercalls against the `name` will result in failure unless
   **XEN_SYSCTL_XLIVE_PATCH_UPLOAD** hypercall is perfomed with same `name`.
  * *XLIVE_PATCH_ACTION_REVERT* (2) revert the payload. If the operation takes
  more time than the upper bound of time the `rc` in `xen_xlive_patch_status'
  retrieved via **XEN_SYSCTL_XLIVE_PATCH_GET** will be -XEN_EBUSY.
  * *XLIVE_PATCH_ACTION_APPLY* (3) apply the payload. If the operation takes
  more time than the upper bound of time the `rc` in `xen_xlive_patch_status'
  retrieved via **XEN_SYSCTL_XLIVE_PATCH_GET** will be -XEN_EBUSY.
  * *XLIVE_PATCH_ACTION_REPLACE* (4) revert all applied payloads and apply this
  payload. If the operation takes more time than the upper bound of time
  the `rc` in `xen_xlive_patch_status' retrieved via **XEN_SYSCTL_XLIVE_PATCH_GET**
  will be -XEN_EBUSY.
...
<pre>
#define XLIVE_PATCH_ACTION_UNLOAD  1  
#define XLIVE_PATCH_ACTION_REVERT  2  
#define XLIVE_PATCH_ACTION_APPLY   3  
#define XLIVE_PATCH_ACTION_REPLACE 4  
struct xen_sysctl_xlive_patch_action {  
    xen_xlive_patch_name_t name;                /* IN, name of the patch. */  
    uint32_t cmd;                           /* IN: XLIVE_PATCH_ACTION_* */  
...
## State diagrams of XLIVE_PATCH_ACTION commands.

There is a strict ordering state of what the commands can be.
The XLIVE_PATCH_ACTION prefix has been dropped to easy reading and
does not include the XLIVE_PATCH_STATES:
...
## State transition table of XLIVE_PATCH_ACTION commands and XLIVE_PATCH_STATUS.

Note that:

 - The CHECKED state is the starting one achieved with *XEN_SYSCTL_XLIVE_PATCH_UPLOAD* hypercall.
...
The normal sequence of events is to:
 ...
 1. *XEN_SYSCTL_XLIVE_PATCH_UPLOAD* to upload the payload. If there are errors *STOP* here.
 2. *XEN_SYSCTL_XLIVE_PATCH_GET* to check the `->rc`. If *-XEN_EAGAIN* spin. If zero go to next step.
 3. *XEN_SYSCTL_XLIVE_PATCH_ACTION* with *XLIVE_PATCH_ACTION_APPLY* to apply the patch.
 4. *XEN_SYSCTL_XLIVE_PATCH_GET* to check the `->rc`. If in *-XEN_EAGAIN* spin. If zero exit with success.
...
### Generation of Xen Live Patch ELF payloads
...
Currently it resides at https://github.com/rosslagerwall/xlive_patch-build
...
### Xen Live Patch interdependencies

Xen Live Patch patches interdependencies are tricky.
...
This is for further development of Xen Live Patch.
 ...
 * Further safety checks (blacklist of which functions cannot be patched, check
   ...
   Specifically we want to make sure that Xen Live Patch codepaths cannot be patched.

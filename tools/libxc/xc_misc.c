...
int xc_xhot_patch_upload(xc_interface *xch,
                         char *name,
                         ...
                         uint32_t size)
{
    ...
    xen_xhot_patch_name_t def_name = { .pad = { 0, 0, 0 } };
    ...
    if ( def_name.size > XEN_XHOT_PATCH_NAME_SIZE )
    {
        errno = EINVAL;
        return -1;
    }
    ...
    sysctl.cmd = XEN_SYSCTL_xhot_patch_op;
    sysctl.u.xhot_patch.cmd = XEN_SYSCTL_XHOT_PATCH_UPLOAD;
    sysctl.u.xhot_patch.pad = 0;
    sysctl.u.xhot_patch.u.upload.size = size;
    set_xen_guest_handle(sysctl.u.xhot_patch.u.upload.payload, local);

    sysctl.u.xhot_patch.u.upload.name = def_name;
    set_xen_guest_handle(sysctl.u.xhot_patch.u.upload.name.name, name);
...
int xc_xhot_patch_get(xc_interface *xch,
                      char *name,
                      xen_xhot_patch_status_t *status)
{
    ...
    sysctl.cmd = XEN_SYSCTL_xhot_patch_op;
    sysctl.u.xhot_patch.cmd = XEN_SYSCTL_XHOT_PATCH_GET;
    sysctl.u.xhot_patch.pad = 0;

    sysctl.u.xhot_patch.u.get.status.state = 0;
    sysctl.u.xhot_patch.u.get.status.rc = 0;

    sysctl.u.xhot_patch.u.get.name = def_name;
    set_xen_guest_handle(sysctl.u.xhot_patch.u.get.name.name, name);

    rc = do_sysctl(xch, &sysctl);

    xc_hypercall_bounce_post(xch, name);

    memcpy(status, &sysctl.u.xhot_patch.u.get.status, sizeof(*status));

    return rc;
}

/*
 * The heart of this function is to get an array of xen_xhot_patch_status_t.
 *
 ...
 * be filled up with.
 *
 * Each entry in the 'name' array is expected to be of XEN_XHOT_PATCH_NAME_SIZE
 * length.
 *
 * Each entry in the 'info' array is expected to be of xen_xhot_patch_status_t
 * structure size.
 *
 ...
 * retrieved (if any).
 */
int xc_xhot_patch_list(xc_interface *xch, unsigned int max, unsigned int start,
                       xen_xhot_patch_status_t *info,
                       ...
                       unsigned int *left)
{
    ...
    sysctl.cmd = XEN_SYSCTL_xhot_patch_op;
    sysctl.u.xhot_patch.cmd = XEN_SYSCTL_XHOT_PATCH_LIST;
    sysctl.u.xhot_patch.pad = 0;
    sysctl.u.xhot_patch.u.list.version = 0;
    sysctl.u.xhot_patch.u.list.idx = start;
    sysctl.u.xhot_patch.u.list.pad = 0;

    max_batch_sz = max;
    /* Convience value. */
    sz = sizeof(*name) * XEN_XHOT_PATCH_NAME_SIZE;
    *done = 0;
    *left = 0;
    do {
        /*
        ...
        sysctl.u.xhot_patch.u.list.nr = nr;
        ...
        set_xen_guest_handle(sysctl.u.xhot_patch.u.list.status, info);
        set_xen_guest_handle(sysctl.u.xhot_patch.u.list.name, name);
        set_xen_guest_handle(sysctl.u.xhot_patch.u.list.len, len);
        ...
        if ( !version )
            version = sysctl.u.xhot_patch.u.list.version;

        if ( sysctl.u.xhot_patch.u.list.version != version )
        {
        ...
        *left = sysctl.u.xhot_patch.u.list.nr; /* Total remaining count. */
        ...
        *done += rc;
        /* Update idx. */
        sysctl.u.xhot_patch.u.list.idx = *done;
...
static int _xc_xhot_patch_action(xc_interface *xch,
                                 char *name,
                                 unsigned int action,
                                 uint32_t timeout)
{
    ...
    sysctl.cmd = XEN_SYSCTL_xhot_patch_op;
    sysctl.u.xhot_patch.cmd = XEN_SYSCTL_XHOT_PATCH_ACTION;
    sysctl.u.xhot_patch.pad = 0;
    sysctl.u.xhot_patch.u.action.cmd = action;
    sysctl.u.xhot_patch.u.action.timeout = timeout;

    sysctl.u.xhot_patch.u.action.name = def_name;
    set_xen_guest_handle(sysctl.u.xhot_patch.u.action.name.name, name);
...
int xc_xhot_patch_apply(xc_interface *xch, char *name, uint32_t timeout)
{
    return _xc_xhot_patch_action(xch, name, XHOT_PATCH_ACTION_APPLY, timeout);
}

int xc_xhot_patch_revert(xc_interface *xch, char *name, uint32_t timeout)
{
    return _xc_xhot_patch_action(xch, name, XHOT_PATCH_ACTION_REVERT, timeout);
}

int xc_xhot_patch_unload(xc_interface *xch, char *name, uint32_t timeout)
{
    return _xc_xhot_patch_action(xch, name, XHOT_PATCH_ACTION_UNLOAD, timeout);
}

int xc_xhot_patch_replace(xc_interface *xch, char *name, uint32_t timeout)
{
    return _xc_xhot_patch_action(xch, name, XHOT_PATCH_ACTION_REPLACE, timeout);
}

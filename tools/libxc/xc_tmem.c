                 ...
                 int dom, int io_fd, int live, int field_marker)
{
    ...
    for ( i = 0; i < info.nr_pools; i++ )
    {
        ...
        IPRINTF("%s: %d/%d id=%d raw=%x %s%s %d %d, npages=%ld\n", __func__, i, info.nr_pools, pool->id,
                pool->flags.raw,
 ...
 out_memory:
            ...
            DPRINTF("saved %ld tmem pages for dom=%d pool=%d, checksum=%x\n",
...
int xc_tmem_restore(xc_interface *xch, int dom, int io_fd)
{
 ...
 out_memory:
        ...
        if ( pool.n_pages )
            DPRINTF("restored %ld tmem pages for dom=%d pool=%d, check=%x\n",
                    pool.n_pages - j, dom, pool.id, checksum);
    }
    free(buf);

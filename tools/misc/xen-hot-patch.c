...
{
    fprintf(stderr,
            "xen-xhot_patch: xHot Patch test tool\n"
            "Usage: xen-xhot_patch <command> [args]\n"
...
static int list_func(int argc, char *argv[])
{
    ...
    xen_xhot_patch_status_t *info = NULL;
    char *name = NULL;
    uint32_t *len = NULL;
    int rc = ENOMEM;
    ...
    if ( !info )
        return rc;
    name = malloc(sizeof(*name) * XEN_XHOT_PATCH_NAME_SIZE * MAX_LEN);
    if ( !name )
    {
        free(info);
        return rc;
    ...
    do {
        ...
        memset(name, 'B', sizeof(*name) * MAX_LEN * XEN_XHOT_PATCH_NAME_SIZE);
        memset(len, 'C', sizeof(*len) * MAX_LEN);
        rc = xc_xhot_patch_list(xch, MAX_LEN, idx, info, name, len, &done, &left);
        if ( rc )
        {
        ...
        for ( i = 0; i < done; i++ )
        {
            ...
            sz = len[i];
            str = name + (i * XEN_XHOT_PATCH_NAME_SIZE);
            for ( j = sz; j < XEN_XHOT_PATCH_NAME_SIZE; j++ )
                str[j] = '\0';
...
static int get_name(int argc, char *argv[], char *name)
{
    ...
    if ( len > XEN_XHOT_PATCH_NAME_SIZE )
    {
    ...
    memset(name, 0, XEN_XHOT_PATCH_NAME_SIZE);
...
{
    char *filename;
    char name[XEN_XHOT_PATCH_NAME_SIZE];
    int fd = 0, rc;
    struct stat buf;
    unsigned char *fbuf;
    ssize_t len;
    ...
    rc = xc_xhot_patch_upload(xch, name, fbuf, len);
    if ( rc )
...
struct {
    ...
    {   .allow = XHOT_PATCH_STATE_CHECKED,
        .expected = XHOT_PATCH_STATE_APPLIED,
        .name = "apply",
        .function = xc_xhot_patch_apply,
    },
    {   .allow = XHOT_PATCH_STATE_APPLIED,
        .expected = XHOT_PATCH_STATE_CHECKED,
        .name = "revert",
        .function = xc_xhot_patch_revert,
    },
    {   .allow = XHOT_PATCH_STATE_CHECKED,
        .expected = -XEN_ENOENT,
        .name = "unload",
        .function = xc_xhot_patch_unload,
    },
    {   .allow = XHOT_PATCH_STATE_CHECKED,
        .expected = XHOT_PATCH_STATE_APPLIED,
        .name = "replace",
        .function = xc_xhot_patch_replace,
    },
};
...
int action_func(int argc, char *argv[], unsigned int idx)
{
    ...
    rc = xc_xhot_patch_get(xch, name, &status);
    if ( rc )
    {

...
{
    fprintf(stderr,
            "xen-live-patch: live patching test tool\n"
            "Usage: xen-live-patch <command> [args]\n"
...
static int list_func(int argc, char *argv[])
{
    ...
    xen_live_patch_status_t *info = NULL;
    char *name = NULL;
    uint32_t *len = NULL;
    int rc = ENOMEM;
    ...
    do {
        ...
        rc = xc_live_patch_list(xch, MAX_LEN, idx, info, name, len, &done, &left);
        if ( rc )
        {
...
static int upload_func(int argc, char *argv[])
{
    ...
    rc = xc_live_patch_upload(xch, name, fbuf, len);
    if ( rc )
...
struct {
    ...
    {   .allow = LIVE_PATCH_STATE_CHECKED,
        ...
        .name = "apply",
        .function = xc_live_patch_apply,
    },
    {   .allow = LIVE_PATCH_STATE_APPLIED,
        .expected = LIVE_PATCH_STATE_CHECKED,
        .name = "revert",
        .function = xc_live_patch_revert,
    },
    {   .allow = LIVE_PATCH_STATE_CHECKED,
        .expected = -XEN_ENOENT,
        .name = "unload",
        .function = xc_live_patch_unload,
    },
    {   .allow = LIVE_PATCH_STATE_CHECKED,
        .expected = LIVE_PATCH_STATE_APPLIED,
        .name = "replace",
        .function = xc_live_patch_replace,
    },
};
...
int action_func(int argc, char *argv[], unsigned int idx)
{
    ...
    rc = xc_live_patch_get(xch, name, &status);
    if ( rc )
    {

...
{
    fprintf(stderr,
            "xen-xlive_patch: Xen Live Patch test tool\n"
            "Usage: xen-xlive_patch <command> [args]\n"
...
static int list_func(int argc, char *argv[])
{
    ...
    xen_xlive_patch_status_t *info = NULL;
    char *name = NULL;
    uint32_t *len = NULL;
    int rc = ENOMEM;
    ...
    if ( !info )
        return rc;
    name = malloc(sizeof(*name) * XEN_XLIVE_PATCH_NAME_SIZE * MAX_LEN);
    if ( !name )
    {
        free(info);
        return rc;
    ...
    do {
        ...
        memset(name, 'B', sizeof(*name) * MAX_LEN * XEN_XLIVE_PATCH_NAME_SIZE);
        memset(len, 'C', sizeof(*len) * MAX_LEN);
        rc = xc_xlive_patch_list(xch, MAX_LEN, idx, info, name, len, &done, &left);
        if ( rc )
        {
        ...
        for ( i = 0; i < done; i++ )
        {
            ...
            sz = len[i];
            str = name + (i * XEN_XLIVE_PATCH_NAME_SIZE);
            for ( j = sz; j < XEN_XLIVE_PATCH_NAME_SIZE; j++ )
                str[j] = '\0';
...
static int get_name(int argc, char *argv[], char *name)
{
    ...
    if ( len > XEN_XLIVE_PATCH_NAME_SIZE )
    {
    ...
    memset(name, 0, XEN_XLIVE_PATCH_NAME_SIZE);
...
{
    char *filename;
    char name[XEN_XLIVE_PATCH_NAME_SIZE];
    int fd = 0, rc;
    struct stat buf;
    unsigned char *fbuf;
    ssize_t len;
    ...
    rc = xc_xlive_patch_upload(xch, name, fbuf, len);
    if ( rc )
...
struct {
    ...
    {   .allow = XLIVE_PATCH_STATE_CHECKED,
        .expected = XLIVE_PATCH_STATE_APPLIED,
        .name = "apply",
        .function = xc_xlive_patch_apply,
    },
    {   .allow = XLIVE_PATCH_STATE_APPLIED,
        .expected = XLIVE_PATCH_STATE_CHECKED,
        .name = "revert",
        .function = xc_xlive_patch_revert,
    },
    {   .allow = XLIVE_PATCH_STATE_CHECKED,
        .expected = -XEN_ENOENT,
        .name = "unload",
        .function = xc_xlive_patch_unload,
    },
    {   .allow = XLIVE_PATCH_STATE_CHECKED,
        .expected = XLIVE_PATCH_STATE_APPLIED,
        .name = "replace",
        .function = xc_xlive_patch_replace,
    },
};
...
int action_func(int argc, char *argv[], unsigned int idx)
{
    ...
    rc = xc_xlive_patch_get(xch, name, &status);
    if ( rc )
    {
